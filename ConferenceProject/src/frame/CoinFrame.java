...
public class CoinFrame extends JFrame implements ActionListener{
	...
	public CoinFrame(User user) {
		...
		nickname_label = new JLabel(this.user.getNickname()+"님");//로그인한 id의 nickname
		...
		beforecoin_label = new JLabel(Integer.toString(this.user.getCoin()));//int형 coin을 string으로 변환
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == charge_button) {//charge_button 클릭 시
			Coin coin = new Coin();
			coin.setIdCoin(this.user.getUser_id(), chargecoin_tf.getText());
			try {
				if(coin.updatecoinConnect() == 0) {
					return;
				}else {
					setVisible(false);
